///////////////////////////////////////////////////////////////////////////////////////////////////
// File Name     : 2019202031_semaphore_server.c                                     //
// Date          : 2023/05/27                                            //
...
sem_t *mysem;
FILE * log_file;
const char* log_file_name = "server_log.txt"; //로그파일
...
void alarmHandler(int signum)
{
    ...
    if (signum == SIGINT) // if parent process and signal is SIGINT
    {
        ...
        char buffer[1024];
        sprintf(buffer,"\n[%s] Server is terminated.\n", curtime_str);
        pthread_create(&tidB, NULL, print_log, (void*)buffer);
            ...
            if(pids[i] != 0)
            {
                for(int j = 0; j< MaxChilds; j++)
                {
                    if(pids[i] == shared_memory->Idle_pid_list[j])
                    {
                        ...
                        char buffer[1024];
                        sprintf(buffer,"\n[%s] IdleProcessCount : %d\n[%s] %d Process is terminated.\n", curtime_str, shared_memory->IDLE_process, curtime_str, pids[i]);
                        pthread_create(&tidB, NULL, print_log, (void*)buffer);
                        pthread_join(tidB,NULL);
...
int main(int argc, char **argv)
{
    ...
    char buffer[1024];
    sprintf(buffer,"[%s] Server is started.\n", curtime_str);
    pthread_create(&tidB, NULL, print_log, (void*)buffer);
    pthread_join(tidB,NULL);
...
void child_main(int i, int socket_fd, int addrlen)
{
    ...
    while(1)
    {
        ...
        printf("==========================================\n\n");
        char buffer[1024];
        sprintf(buffer,"=============== New Client ===============\nTIME : [%s] \nURL : %s\nIP : %s\nPort : %d\n",curtime_str,url, IP1, client_addr.sin_port);
        pthread_create(&tidB, NULL, print_log, (void*)buffer);
        pthread_join(tidB,NULL);
        ...
        time_sec = (end_time.tv_sec - start_time.tv_sec) * 1000000 + (end_time.tv_usec - start_time.tv_usec); //총 걸린 시간 계산하기
        ...
        printf("CONNECTIONG TIME: %ld(us)\n", time_sec);
        char buffer1[1024];
        sprintf(buffer1,"========== Disconnected Client ===========\nTIME : [%s] \nURL : %s\nIP : %s\nPort : %d\nCONNECTIONG TIME: %ld(us)\n",curtime_str2,url, IP1, client_addr.sin_port, time_sec);
        pthread_create(&tidB, NULL, print_log, (void*)buffer1);
        pthread_join(tidB,NULL);
...
// Input :  log파일에 넣을 값                                                       //
// Output : x                                  //
// Purpose : 세마포어를 이용한 log파일에 값 쓰기                         //
///////////////////////////////////////////////////////////////////////////////////////////////////
void *print_log(void* vptr)
{
    sem_t* mysem = sem_open("mysem", O_CREAT | O_EXCL,0777,1);
    sem_close(mysem);
    mysem = sem_open("mysem", O_RDWR);
    sem_wait(mysem);
    FILE* fd = fopen(log_file_name,"a");
    char* write_word = (char*)vptr;
    fprintf(fd,"%s",write_word);
    fclose(fd);
    sem_post(mysem);
    return NULL;
}
...
void *doit2(void *vptr)
{
    ...
    char buffer[1024];
    sprintf(buffer,"[%s] %d Process is forked.\n[%s] IdleProcessCount : %d\n", curtime_str, getpid(),curtime_str, shared_memory->IDLE_process);
    pthread_create(&tidB, NULL, print_log, (void*)buffer);
    pthread_join(tidB,NULL);
    pthread_mutex_unlock(&counter_mutex);//뮤택스 락 풀기
    return NULL;
...
void *doit3(void *vptr)
{
    ...
    char buffer[1024];
    sprintf(buffer,"[%s] IdleProcessCount : %d\n", curtime_str, shared_memory->IDLE_process);
    pthread_create(&tidB, NULL, print_log, (void*)buffer);
    pthread_join(tidB,NULL);
    pthread_mutex_unlock(&counter_mutex);//뮤택스 락 풀기
    return NULL;
        ...
        if (pids[i] != 0) // If the child process pid number exists
        {
            ...
            for(int j = 0; j<MaxChilds; j++)
            {
                ...
                if(shared_memory->Idle_pid_list[j] == pids[i])
                {
                    ...
                    char buffer[1024];
                    sprintf(buffer,"\n[%s] %d Process is terminated.\n[%s] IdleProcessCount : %d", curtime_str, pids[i], curtime_str, shared_memory->IDLE_process);
                    pthread_create(&tidB, NULL, print_log, (void*)buffer);
                    pthread_join(tidB,NULL);
                    check = 0;
                    shared_memory->Idle_pid_list[j] = 0;
                    break;
                }
            }
            if(check) {
